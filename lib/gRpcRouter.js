...
class GRpcRouter extends Router {
  ...
  async request(req, res, { mockController, resource, action, errorHandler }) {
    ...
    if (mockController) {
      ...
      controller = this.controllers[`${resource}Controller`] && this.controllers[`${resource}Controller`][action];
    }
    ...
    const requestLocals = getNamespace('request-locals');
    requestLocals.run(async() => {
      requestLocals.bindEmitter(req)
      requestLocals.bind(res)
      requestLocals.set('traceId', req.extras.requestId);
